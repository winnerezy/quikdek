...
export const Sidebar = () => {
  ...
  return (
    <aside
      className={cn(
        "w-[250px] flex h-full max-lg:absolute top-0 left-0 z-50 bg-[--background] flex-col items-center gap-10 cursor-pointer duration-300 ease-in-out lg:fixed ",
        isOpen ? "max-lg:translate-x-0" : "max-lg:-translate-x-60"
      )}
      ref={sidebarRef}
    >
      <IoCloseCircleOutline
        className="text-[--blue] size-8 absolute top-7 left-4 block lg:hidden"
        onClick={() => dispatch(closeSideBar())}
      />
      <Link
        href={"/home"}
        className="relative text-[--light-blue] flex gap-4 mt-36 w-48 h-10 rounded-lg hover:bg-[--hover] items-center"
      >
      ...
      <Link
        href={"/my-decks"}
        className="relative text-[--light-blue] flex gap-4 w-48 h-10 rounded-lg hover:bg-[--hover] items-center"
      >
